...
from sample import get_max_index
...
def interpolate(noise1, noise2, latent_dim):
    lambs = torch.linspace(start=0, end=1, steps=10).unsqueeze(1).repeat(1, latent_dim)
...
if __name__ == "__main__":
    ...
    SAVE_DIR = Path(__file__).parent/"samples/interpolation"
    ...
    for idx in range(max_idx + 1, max_idx + 1 + args.n_iters):
        ...
        noise = interpolate(noise1, noise2, latent_dim=config.LATENT_DIM)
